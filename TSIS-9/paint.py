...
class Button(pygame.sprite.Sprite):
    def __init__(self, points, width=0, color=GRAY):
        ...
        self.rect = pygame.draw.polygon(
            SCREEN, self.color, self.points, self.width)

    def draw(self):
        pygame.draw.polygon(SCREEN, self.color, self.points, self.width)

class Erase(GameObject):
    def __init__(self,start_pos,thickness = 59,color = BLACK):
...
class Rectangle(GameObject):
    ...
    def draw(self):
        ...
        # рисует чет-и
        pygame.draw.rect(SCREEN, self.color, (x1, y1, x2 -
                         x1, y2 - y1), width=self.thickness)
...
def main():
    ...
    Points_er = [(270,10),(290,40),(290,60),(270,50)]
    ...
    switch_eraser = Button(Points_er)
    buttons = [switch_pen,switch_square,switch_triangleR,switch_triangleW,switch_rhombus,switch_eraser]
    current_shape = 'pen'
    objects = []
    active_obj = None
    running = True
